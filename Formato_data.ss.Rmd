...
#head(pelago.aldistSS3)

```

```{r fig.height=5,fig.width=5,echo=FALSE}

clavepelago<-pelago.aldistSS3
Lmed_GE1_pelago<-as.numeric(clavepelago$Lbin_hi)*as.numeric(clavepelago$`1`)
Lmed_GE2_pelago<-as.numeric(clavepelago$Lbin_hi)*as.numeric(clavepelago$`2`)

LMED_GE1_pelago<-clavepelago %>% 
                 mutate(ytr=as.yearqtr(year + step/4))%>%
                 mutate(L1=Lmed_GE1_pelago) %>% 
                  group_by(ytr) %>% 
                summarize(sum1 = sum(`1`),sum2 = sum(L1)) %>% 
                mutate(Lmed=sum2/sum1)

LMED_GE2_pelago<-clavepelago %>% 
                 mutate(ytr=as.yearqtr(year + step/4))%>%
                 mutate(L2=Lmed_GE2_pelago) %>% 
                  group_by(ytr) %>% 
                summarize(sum1 = sum(`2`),sum2 = sum(L2)) %>% 
                mutate(Lmed=sum2/sum1)

ggplot(LMED_GE1_pelago,aes(x=ytr,y=Lmed))+ 
  geom_point()+
   geom_smooth()+
  #geom_line()+ 
  lims(y=c(5,15))+
    ...
    theme(panel.background = element_rect(fill ="gray99")) + 
    theme(panel.grid=element_line(color=NA)) + 
     ggtitle('Talla media del Grupo de edad 1 - Campaña Pelago')+
    theme(plot.title = element_text(size = 10))+ 
    theme_bw(base_size=8) +
scale_x_yearqtr(format = "%Y-T%q")

ggplot(LMED_GE2_pelago,aes(x=ytr,y=Lmed))+ 
  geom_point()+
   geom_smooth()+
  #geom_line()+ 
  lims(y=c(5,15))+
    ...
    theme(panel.background = element_rect(fill ="gray99")) + 
    theme(panel.grid=element_line(color=NA)) + 
     ggtitle('Talla media del Grupo de edad 2 - Campaña Pelago')+
    theme(plot.title = element_text(size = 10))+ 
    theme_bw(base_size=8) +
scale_x_yearqtr(format = "%Y-T%q")
...
AGE<-unique(pelago.aldist_xxx$age)
...
claveecocadiz<-ecocadiz.aldistSS3
Lmed_GE1_ecocadiz<-as.numeric(claveecocadiz$Lbin_hi)*as.numeric(claveecocadiz$`0`)
Lmed_GE2_ecocadiz<-as.numeric(claveecocadiz$Lbin_hi)*as.numeric(claveecocadiz$`1`)

LMED_GE1_ecocadiz<-claveecocadiz %>% 
                 mutate(ytr=as.yearqtr(year + step/4))%>%
                 mutate(L1=Lmed_GE1_ecocadiz) %>% 
                  group_by(ytr) %>% 
                summarize(sum1 = sum(`1`),sum2 = sum(L1)) %>% 
                mutate(Lmed=sum2/sum1)

LMED_GE2_ecocadiz<-claveecocadiz %>% 
                 mutate(ytr=as.yearqtr(year + step/4))%>%
                 mutate(L2=Lmed_GE2_ecocadiz) %>% 
                  group_by(ytr) %>% 
                summarize(sum1 = sum(`2`),sum2 = sum(L2)) %>% 
                mutate(Lmed=sum2/sum1)

ggplot(LMED_GE1_ecocadiz,aes(x=ytr,y=Lmed))+ 
  geom_point()+
   geom_smooth()+
  #geom_line()+ 
  lims(y=c(5,15))+
...
ggplot(LMED_GE2_ecocadiz,aes(x=ytr,y=Lmed))+ 
  geom_point()+
   geom_smooth()+
  #geom_line()+ 
  lims(y=c(5,15))+
...
AGE<-unique(ecocadiz.aldist_xxx$age)
...
AGE<-unique(fleet.ldist.alkseine_xxx$age)
