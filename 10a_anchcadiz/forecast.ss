...
# Modelo configurado por María José Zúñiga en Agosto 2022

# for all year entries except rebuilder; enter either: actual year, -999 for styr, 0 for endyr, neg number for rel. endyr
1 # Benchmarks: 0=skip; 1=calc F_spr,F_btgt,F_msy 
2 # MSY: 1= set to F(SPR); 2=calc F(MSY); 3=set to F(Btgt); 4=set to F(endyr) 
0.6 # SPR target (e.g. 0.40)
0.55 # Biomass target (e.g. 0.40)
...
0.6 # Control rule Biomass level for constant F (as frac of Bzero, e.g. 0.40); (Must be > the no F level below) 
0.55 # Control rule Biomass level for no F (as frac of Bzero, e.g. 0.10) 
...
1989 # Rebuilder:  first year catch could have been set to zero (Ydecl)(-1 to set to 1999)
